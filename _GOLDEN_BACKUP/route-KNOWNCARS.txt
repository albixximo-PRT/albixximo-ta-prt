...
async function callOcrSpace(apiKey: string, jpegBuffer: Buffer, engine: "1" | "2") {
  ...
  for (const endpoint of OCR_PRO_ENDPOINTS) {
    ...
    try {
      ...
      fd.append("file", new Blob([new Uint8Array(jpegBuffer)], { type: "image/jpeg" }), "gt7.jpg")
...
export async function POST(req: NextRequest) {
  try {
    ...
    const qualiPosMapsTimesOnly = qualiTimesOnlyTexts.map((t) => extractQualiTimesByPos(t))
    const qualiByPosTimesOnly = mergeQualiPosMaps(...qualiPosMapsTimesOnly)

    const qualiPosMapsAll = qualiTexts.map((t) => extractQualiTimesByPos(t))
    const qualiByPosAll = mergeQualiPosMaps(...qualiPosMapsAll)
  ...
  const qualiAutoByPilot = normalizeKnownCar(
    (qExact?.auto || "").trim() ||
    (qLoose?.auto || "").trim()
  )
